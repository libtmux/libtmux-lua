...
def main():
    ...
    try:
        ...
        for adapter, entry in (("luv", "run"), ("nvim", "start")):
            ...
            complete('  server:', {"snapshot", "close", "handle", "command", "group", "batch", "new_session", "query_panes", "explain_panes"})
            ...
            complete('  created_session:', {"new_window", "reference"})
            lines.append('  local created_pane = created.pane')
            complete('  created_pane:', {"split", "reference"})
