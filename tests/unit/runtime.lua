...
function tests.test_root_operation_survives_borrower_cancel_before_and_after_settlement()
    for _, late in ipairs({ false, true }) do
        local rt, driver = fixture({ max_active = 1 })
        lu.assertEquals(type(rt._root_operation), "function", "root-owned operations are missing")
        local first, producer, child, second_value, cleanup
        local root = rt:start(function()
            first = rt:_operation(function()
                producer = rt:_root_operation(function(_, request)
                    assert(rt:_resource(function(done)
                        cleanup = done
                    end))
                    lu.assertTrue(request:_retain(4))
                    child = operation(rt)
                    return child.request:await()
                end)
                return producer:await()
            end)
            rt:spawn(function()
                second_value = producer:await()
            end)
            return "joined"
        end)
        driver:drain()
        if late then
            producer:_on_retire(function()
                lu.assertTrue(first:cancel())
            end)
        else
            lu.assertTrue(first:cancel())
            driver:drain()
        ...
        lu.assertEquals(child.stopped, 0)
        child.settle("ready")
        driver:drain()
        lu.assertFalse(producer:is_retired())
        child.retire()
        driver:drain()
        local _, err = first:result()
        lu.assertEquals(err.code, "cancelled")
        lu.assertEquals(second_value, "ready")
        lu.assertEquals(child.stopped, 0)
        lu.assertFalse(root:is_retired())
        lu.assertEquals(type(cleanup), "function")
        cleanup()
        lu.assertTrue(root:is_retired())
        lu.assertEquals(root:result(), "joined")
        lu.assertEquals(rt:stats().bytes, 0)
        lu.assertEquals(rt:stats().resources, 0)
    end
end

function tests.test_root_operation_errors_do_not_cancel_unrelated_work()
    local rt, driver = fixture()
    lu.assertEquals(type(rt._root_operation), "function", "root-owned operations are missing")
    local marker = { code = "startup_failed" }
    local child, producer, seen
    local root = rt:start(function()
        child = operation(rt)
        rt:_operation(function()
            producer = rt:_root_operation(function()
                error(marker)
            end)
            local _, err = producer:await()
            seen = err
        end)
        return "continued"
    end)
    driver:drain()
    lu.assertIs(seen, marker)
    lu.assertEquals(child.stopped, 0)
    lu.assertFalse(root:is_settled())
    child.settle(true)
    child.retire()
...
function tests.test_root_operation_abort_keeps_child_retirement_and_rejects_late_work()
    local rt, driver = fixture({ max_tasks = 3 })
    lu.assertEquals(type(rt._root_operation), "function", "root-owned operations are missing")
    local producer, child, full
    local root = rt:start(function()
        rt:_operation(function()
            producer = rt:_root_operation(function()
                child = operation(rt)
                return child.request:await()
            end, { operation = "shared.start", effect = "unknown" })
            full = rt:_root_operation(function()
    ...
    driver:drain()
    local _, err = full:result()
    lu.assertEquals(err.code, "queue_full")
    root:cancel("shutdown")
    driver:drain()
    local _, producer_error = producer:result()
    lu.assertEquals(producer_error.operation, "shared.start")
    lu.assertEquals(producer_error.effect, "unknown")
    lu.assertEquals(child.stopped, 1)
    lu.assertFalse(producer:is_retired())
    lu.assertFalse(root:is_retired())
    local late = rt:_root_operation(function()
        error("must not start")
    end)
    local _, late_error = late:result()
    lu.assertEquals(late_error.code, "closed")
    child.retire()
    driver:drain()
    lu.assertTrue(producer:is_retired())
    lu.assertTrue(root:is_retired())
    lu.assertEquals(rt:stats().tasks, 0)
end
